...
void Cube::draw() const {
    ...
    // purple
    glColor3f(238/255, 130/255, 238/155);
    draw_point(corners[4]);
    draw_point(corners[0]);
    draw_point(corners[1]);
    draw_point(corners[5]);
...
void Cube::rotate(double theta_x, double theta_y, double theta_z) {
    ...
    // Rotate x (3d->2d: y->x, z->y)
    /* in 2d coordinates, b.x = a.x*cos(angle)  - a.y*sin(angle)
...
void Cube::shrink(double delta_size) {
    edge_length += edge_length * delta_size;
    reset_corners();
}